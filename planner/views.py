...
class SelectHotelWithMapView(LoginRequiredMixin, View):
    ...
    def get(self, request, trip_id):
        ...
        # Use destination coordinates when present; otherwise fall back to first hotel with coords or default Myanmar center
        center_lat = None
        center_lng = None
    ...
    def get(self, request, destination_id):
        ...
        for hotel in hotels:
            ...
            hotel_data.append({
                ...
                'category': hotel.category,
                'category_display': hotel.get_category_display(),
                'type': hotel.get_category_display(),
        ...
        if hotel_id:
            try:
                ...
                if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                    ...
                    return redirect(redirect_url)  # Changed to redirect to main plan page
...
class PlanSelectionView(LoginRequiredMixin, View):
    ...
    def get(self, request, trip_id):
        ...
        hotel_total_cost_numeric = 0
        hotel_cost_per_night_numeric = 0

        hotel_total_cost_mmk = "Not selected"
        hotel_cost_per_night_mmk = "Not selected"

        if selected_hotel:
            try:
                price = float(selected_hotel.price_per_night)

                hotel_cost_per_night_numeric = price
                hotel_total_cost_numeric = price * nights

                hotel_cost_per_night_mmk = f"{price:,.0f} MMK"
                hotel_total_cost_mmk = f"{hotel_total_cost_numeric:,.0f} MMK"

            except:
                hotel_total_cost_mmk = "Error"
                hotel_cost_per_night_mmk = "Error"
        ...
        if selected_transport:
            ...
            try:
                ...
                print("DEBUG Transport:", selected_transport)
                ...
                if transport_type in ['bus', 'flight']:
                    ...
                    if total_price:
                        ...
                        print("DEBUG: Using booking_details total:", total_price)
        ...
        total_combined_cost_numeric = (
            hotel_total_cost_numeric + transport_cost_numeric
        )
        ...
        context = {
            ...
            # HOTEL
            'hotel_total_cost_mmk': hotel_total_cost_mmk,
            'hotel_cost_per_night_mmk': hotel_cost_per_night_mmk,

            'hotel_total_cost_numeric': hotel_total_cost_numeric,
            'hotel_cost_per_night_numeric': hotel_cost_per_night_numeric,
        ...
        print("DEBUG FINAL")
        print("Hotel:", hotel_total_cost_mmk)
        print("Transport:", transport_cost_mmk)
        print("Total:", total_combined_cost_mmk)
    ...
    def _build_activities_from_attractions(self, destination, preferred_types, default_type, time_slots, fallback, day_offset=0):
        """Create activity list from attractions for a destination; fall back to provided list when none."""
        attractions = list(
            Attraction.objects.filter(destination=destination, is_active=True).order_by('name')
        )

        if not attractions:
            return fallback

        def type_priority(attraction):
            atype = (attraction.type or '').lower()
            return 0 if any(pref in atype for pref in preferred_types) else 1

        sorted_attractions = sorted(attractions, key=lambda a: (type_priority(a), a.name))

        activities = []
        slot_count = len(time_slots)
        start = (day_offset * slot_count) % len(sorted_attractions)

        for idx in range(slot_count):
            attr = sorted_attractions[(start + idx) % len(sorted_attractions)]
            activities.append({
                'time': time_slots[idx],
                'title': attr.name,
                'location': attr.destination.name,
                'duration': '1.5 hours',
                'description': attr.description or f'Visit {attr.name}',
                'type': (attr.type or default_type).lower(),
            })

        return activities if activities else fallback

    def generate_cultural_itinerary(self, trip, days):
        """Generate cultural itinerary for specific destination"""
        itinerary = []
        cultural_time_slots = ['09:00 AM', '11:30 AM', '02:00 PM', '04:30 PM']
        ...
        for day in range(1, days + 1):
            base_activities = self._build_activities_from_attractions(
                destination=trip.destination,
                preferred_types=['cultural', 'religious', 'pagoda', 'temple', 'heritage', 'museum', 'monastery', 'historical'],
                default_type='cultural',
                time_slots=cultural_time_slots,
                fallback=[
    ...
    def generate_adventure_itinerary(self, trip, days):
        ...
        itinerary = []
        adventure_time_slots = ['06:30 AM', '10:30 AM', '02:30 PM', '05:30 PM']
        ...
        for day in range(1, days + 1):
            base_activities = self._build_activities_from_attractions(
                destination=trip.destination,
                preferred_types=['adventure', 'hiking', 'mountain', 'trek', 'cave', 'waterfall', 'beach', 'lake', 'boat', 'cycling', 'river', 'island'],
                default_type='adventure',
                time_slots=adventure_time_slots,
                fallback=[
    ...
    def generate_relaxed_itinerary(self, trip, days):
        ...
        itinerary = []
        relaxed_time_slots = ['09:30 AM', '12:00 PM', '03:00 PM', '05:30 PM']
        ...
        for day in range(1, days + 1):
            base_activities = self._build_activities_from_attractions(
                destination=trip.destination,
                preferred_types=['wellness', 'spa', 'park', 'garden', 'beach', 'lake', 'view', 'viewpoint', 'photography'],
                default_type='relaxation',
                time_slots=relaxed_time_slots,
                fallback=[
